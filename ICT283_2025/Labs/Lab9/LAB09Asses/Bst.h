...
template <typename T> void Bst<T>::Insert(T newData) {
  ...
  while (current != nullptr) {
    ...
    if (!(current->info < newData) && !(newData < current->info)) {
      // equivalent to == when only < is defined
      return;
    } else if (newData < current->info) {
      current = current->left;
    } else {
      current = current->right;
    }
  }
  if (newData < trailCurrent->info) {
...
template <typename T> void Bst<T>::printInOrder(PN processNode) {
  if (root == nullptr) {
...
template <typename T> bool Bst<T>::SearchTree(const T &target) {
