...
template <class T>
using Bst = BinarySearchTree<T>;  // same thing with shorter name, can use Bst below

#include <iostream>

using namespace std; // <-----  DON'T DO THIS ANYMORE. don't expose this namespace
						// LOOK AT EXAMPLE IN Timing Fibonacci.cpp
...
//  So there are no setters for convenience of demo.
class unit
{
public:
    unit()
...
// Think of what you had to do with your Date object when you wanted to insert
// into a BST.
...
// Const & parameter?
// what if this routine changes the data. 
// Would the BST's invariant property
// be guaranteed? If the invariant property of a data structure is not guaranteed
// the data structure is BROKEN - CANNOT BE USED
...
public:
    static void f1u( unit &data)  // quick and dirty - inlined for demo only.
								  // **DO NOT INLINE YOUR METHODS IN LABS/ASSIGNMENT/EXAM. Separate interface from implementation **
								  // This is the callback called by the tree
...
int test1()
{
    Bst<int>  intTree; // change the name as appropriate for your bst. Shortened name used because of the using statement.
